...
/*
*
*   FILE: GET ROUTE BY PART FOR SORT
*   AUTHOR: ROBERT SMITH
*   LAST MODIFIED: 11/21/2017
*   NOTE: THE ONLY DIFFERENCE IN THIS AND GET ROUTE BY PART IS THAT THIS FILE RETURNS AND ARRAY AS A OBJECT
*   FUNCTIONALITY: 
*               1). GET CONNECTED
*               2). STORE VARIABLES
            ...
            if($res){//SUCCESS
                ...
                if($rows_affected != 0){
                    ...
                    while($row = sqlsrv_fetch_array($res, SQLSRV_FETCH_ASSOC)){
                        //PUSH TO AN ARRAY
                        array_push($dataToSendBack, array(  "mcode" => $row['mcode'],
                                                            "process_desc" => $row['p_desc'],
                                                            "process_seq" => $row['process_sequence'])
                                  );
                    }
                    //SEND BACK TO SCRIPT
                    echo json_encode($dataToSendBack);
                }else{
                    $dataToSendBack = array("error" => "no route");                    
                    echo json_encode($dataToSendBack);   
                    exit;
                }               
            } else {//QUERY FAILED
